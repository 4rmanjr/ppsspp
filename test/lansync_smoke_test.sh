...
PORT=27314
TMP_HOME=$(mktemp -d)
STATE_DIR="$TMP_HOME/.ppsspp/PSP/PPSSPP_STATE"
mkdir -p "$STATE_DIR"
...
HOME="$TMP_HOME" "$PPSSPP_BIN" --lansync-enabled --lansync-port="$PORT" &
PID=$!
sleep 2
kill -0 "$PID" 2>/dev/null || { echo "FAIL: process died"; exit 1; }
echo "PASS"
...
TEST_FILE="$STATE_DIR/ULUS12345_0.ppst"
