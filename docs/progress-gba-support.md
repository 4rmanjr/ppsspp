...
| GBA Fast-forward & Pause buttons | ✅ **FIXED (ed32432)** | System buttons (Fast-forward, Pause) dari shared TouchControlConfig. Visibility toggles di popup + button rendering di EmuScreen GBA path. Pause pakai `&pauseTrigger_`, Fast-forward pakai `&PSP_CoreParameter().fastForward`. Disabled Pause toggle di device tanpa hardware back button (match PSP). |
