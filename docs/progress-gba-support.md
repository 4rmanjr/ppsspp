...
| GBA ZIP loading from GameBrowser | ✅ **FIXED** | `DetectType()` hanya cek `.gba`/`.gb`/`.gbc`, bukan `.zip` → PSP path → error. GameBrowser intercept `.zip` ke InstallZipScreen. Fix: (1) `EmuCore.cpp` `DetectType()` cek isi ZIP pake libzip. (2) `MainScreen.cpp` `HasGBAROM()` + `LaunchFile()` route GBA-zips langsung ke EmuScreen. (3) `EmuScreen.cpp` passing `EmuCore::Type::GBA`. Dual-build ON/OFF verified. **Tambahan:** `.gb` (3 chars) juga didukung di dalam ZIP — semua 3 file ZIP detection (EmuCore.cpp, GBACore.cpp, MainScreen.cpp) sekarang deteksi `.gb`, `.gba`, `.gbc`. |
